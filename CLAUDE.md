...
Project-specific instructions for termcore. The global style guide referenced in `~/.claude/CLAUDE.md` remains authoritative; this file only records what is specific to this repository.

## What this project is

termcore is a library, not an application. It is the foundation under termplate, which section 3.2.9 of the style guide names as the template every Textual-based TUI is checked against.
...
- No boolean parameter selects behaviour anywhere in the public API. Section 1.1.2 is enforced by `FBT`, and the established answers are two named functions (`next_index` and `clamped_index`, `timing` and `timing_ns`), a classmethod factory (`ThemeLoader.custom_only`) or an enum (`DateFormat`, `HelpScope`). Pick whichever fits the parameter; a format selector is an enum, a behaviour switch is two functions. A fourth answer appeared with `MultiLineFooter(groups=...)`: where the two behaviours differ only in whether data was supplied, the presence of that data selects the mode and no extra parameter is needed.
- Domain exceptions live in `termcore/io/errors.py`. A library that calls `sys.exit()` or prints an error leaves its caller no way to react – `AppStateStorage` did both until recently.
- Diagnostics go to a module logger with lazy `%`-formatting, never to `print`. `termcore/cli/output.py` is the one module where printing is the product, and it carries a per-file ignore saying so.
...
- Textual reads annotations at runtime, so type-only imports stay at module level under `termcore/tui/`. The per-file ignore for `TC002` and `TC003` in `pyproject.toml` is that rule, not an oversight.
...
- `docs/DEVELOPMENT.md` holds the module tree, which shared helper already solves which problem, how to exercise a library that has no entry point, and the release procedure. `docs/CLEAN-UP.md` is the checklist for a structured review pass. Both are guarded by `tests/test_docs.py`, so a path that moves fails the suite.

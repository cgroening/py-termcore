"""
Converting between UNIX timestamps and date strings.

This module offers convenient static methods for working with dates and times,
including:

- Converting UNIX timestamps to formatted date strings and vice versa.
- Calculating the number of days between two timestamps.
- Retrieving today's date or timestamp at midnight.

It supports both German-style date formatting ("DD.MM.YYYY") and ISO-style
formatting ("YYYY-MM-DD") for flexibility in various locales, selected through
the `DateFormat` enum.

Every conversion is anchored in the machine's local time zone, and says so:
the datetimes built here carry their offset instead of leaving it implied. A
naive timestamp changes meaning with the daylight saving switch, with the
machine and on serialisation, and the error surfaces months later in wrongly
sorted or wrongly calculated data rather than at the call site.

"""

from datetime import UTC, datetime, time
from enum import Enum

# Without this, `from .datetime import *` in the package __init__ re-exports
# the imported `datetime` class, which then shadows this very module: after
# importing termz, `termz.util.datetime` is the class, not the module.
__all__ = [
    "DateFormat",
    "date_diff",
    "date_to_timestamp",
    "timestamp_to_date",
    "today_date",
    "today_timestamp",
]


class DateFormat(Enum):
    """
    The date layouts these functions read and write.

    Values are `strftime` patterns, so they can be handed to `strftime` and
    `strptime` directly.
    """

    GERMAN = "%d.%m.%Y"
    ISO = "%Y-%m-%d"


def timestamp_to_date(
    timestamp: int | None, date_format: DateFormat = DateFormat.GERMAN
) -> str:
    """
    Converts a UNIX timestamp into a date string.

    Parameters
    ----------
    timestamp : int or None
        UNIX timestamp.
    date_format : DateFormat, optional
        Layout of the returned string, German by default.

    Returns
    -------
    str
        The date as seen in the local time zone. Empty string if the given
        timestamp is None.
    """
    # Return empty string if the given timestamp is None
    if timestamp is None:
        return ""

    # Convert timestamp to a date in the local zone
    date_obj = datetime.fromtimestamp(timestamp, tz=UTC).astimezone()

    return date_obj.strftime(date_format.value)


def date_to_timestamp(
    date_str: str, date_format: DateFormat = DateFormat.GERMAN
) -> int | None:
    """
    Converts a date string into a UNIX timestamp.

    The date is read as local midnight.

    Parameters
    ----------
    date_str : str
        The date, laid out as `date_format` describes.
    date_format : DateFormat, optional
        Layout the string is expected in, German by default.

    Returns
    -------
    int or None
        Unix timestamp (number of seconds since 1970-01-01), or None if the
        string does not match the expected layout.
    """
    # Check if the date string matches the expected format
    try:
        date_obj = datetime.strptime(date_str, date_format.value)  # noqa: DTZ007
    except ValueError:
        return None

    # astimezone() reads a naive value as local time and attaches the matching
    # offset - the assumption this module makes explicit rather than inherits
    return int(date_obj.astimezone().timestamp())


def date_diff(timestamp1: int, timestamp2: int) -> int:
    """
    Calculates the number of calendar days between two timestamps.

    Counted as local calendar dates rather than as 86400-second blocks, so
    the time of day does not matter and a span crossing a daylight saving
    switch is not off by one. Two moments on the same local day are 0 days
    apart, whether they are a second or twenty hours apart.

    Parameters
    ----------
    timestamp1 : int
        UNIX time stamp 1.
    timestamp2 : int
        UNIX time stamp 2.

    Returns
    -------
    int
        Number of days between the given timestamps, negative when the
        second one is the later of the two.
    """
    first = datetime.fromtimestamp(timestamp1, tz=UTC).astimezone().date()
    second = datetime.fromtimestamp(timestamp2, tz=UTC).astimezone().date()

    return (first - second).days


def today_timestamp() -> int:
    """
    Returns the time stamp of today for the time 00:00 h.

    Returns
    -------
    int
        UNIX timestamp of local midnight.
    """
    # Date of today in the local zone
    today = datetime.now(UTC).astimezone().date()

    # Combine with time 00:00 and anchor it in the local zone
    midnight = datetime.combine(today, time.min).astimezone()

    # Return unix timestamp
    return int(midnight.timestamp())


def today_date(date_format: DateFormat = DateFormat.GERMAN) -> str:
    """
    Returns the date of today as a string.

    Parameters
    ----------
    date_format : DateFormat, optional
        Layout of the returned string, German by default.

    Returns
    -------
    str
        Today's date in the local time zone.
    """
    return timestamp_to_date(today_timestamp(), date_format)
